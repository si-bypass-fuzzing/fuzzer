...
async function parseAndSaveIDLFile(sourceFilePath, destFilePath) {
    try {
        ...
        const preprocessedData = commentPreprocessorDirectives(data);
...
async function processInput(inputPath, outputPath) {
    ...
    if (stats.isFile() && path.extname(inputPath) === '.idl') {
        ...
        const destFilePath = outputPath ? outputPath : inputPath.replace('.idl', '.json');
        ...
        for (let file of files) {
            if (path.extname(file) === '.idl') {
                ...
                const destFilePath = path.join(outputPath, path.basename(file, '.idl') + '.json');
...
if (process.argv.length < 3) {
    ...
    console.log("input_path can be a .idl file or a directory containing .idl files.");
    console.log("output_path is optional for a single file, mandatory if input_path is a directory.");
    process.exit(1);
}
