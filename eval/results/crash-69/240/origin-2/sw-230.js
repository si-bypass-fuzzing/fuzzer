...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '2dc37092');
...
async function func0_func0(event) {
			try {
				check_site_violation('http://127.0.0.2:8080', location.origin, 'e2033bcb');
			...
			try {
				var func0_func0_var4 = await event.waitUntil(func0_func0_var3);
			} catch (e) {console.log(e)}

			try {
				var func0_func0_var5 = await event.stopPropagation();
			} catch (e) {console.log(e)}
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'bbf5cdc5');
		...
		try {
			var func1_var6 = await event.composedPath();
		} catch (e) {console.log(e)}
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '8dcf422b');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '50de153c');
		...
		try {
			var func3_var7 = await func3_var6.focus();
		} catch (e) {console.log(e)}

		try {
			var func3_var8 = await self.cookieStore.dispatchEvent(event);
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-654fdaf6] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-9eec3b9d] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'caccf7c3');
		...
		try {
			var func4_var4 = await self.cookieStore.subscribeToChanges(func4_var3);
		} catch (e) {console.log(e)}
		...
		try {
			event.respondWith(fetch('http://127.0.0.2:8080/input-230_page-1.html#2b8c30ad').then((resp) => {if (resp.ok) {return resp;} else {return fetch(event.request);}}).catch((err) => {return fetch(event.request);}));
		} catch (e) {console.log(e)}
	}
	try {
		self.onfetch = func4;
