...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '135407fb');
		...
		try {
			var func0_var3 = await self.clients.claim();
		} catch (e) {console.log(e)}
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '101c81e0');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '511922d3');
		...
		try {
			var func2_var2 = 'starts-with';
		} catch (e) {console.log(e)}

		try {
			var func2_var1 = {name: self.origin, url: self.origin, matchType: func2_var2, };
		} catch (e) {console.log(e)}

		try {
			var func2_var3 = await self.cookieStore.get(func2_var1);
		} catch (e) {console.log(e)}

		try {
			var func2_var4 = 'http://127.0.0.2:8080/input-627_page-2.html#d27487f2';
		} catch (e) {console.log(e)}
...
async function func3(event) {
		...
		try {
			var func3_var3 = 'http://127.0.0.1:8080/input-627_page-1.html#e6557e22';
		} catch (e) {console.log(e)}

		try {
			var func3_var4 = await self.cookieStore.getAll(func3_var3);
		} catch (e) {console.log(e)}
		...
		try {
			var func3_var6 = await self.cookieStore.subscribeToChanges(func3_var2);
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-c65623c0] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-764d6e8d] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '5478bf2d');
