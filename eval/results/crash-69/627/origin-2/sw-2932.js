...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '31010dc6');
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '1c8b9c9d');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '300421fc');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '44bdba88');
		...
		try {
			var func3_var5 = {name: func3_var1, domain: func3_var1, sameSite: func3_var6, };
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-ab6e4cee] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-2efad7a8] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '5222d92f');
		...
		try {
			var func4_var3 = await event.initEvent(func4_var2);
		} catch (e) {console.log(e)}
