...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '4157b358');
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '8ebb33ee');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'df6a7991');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '0b6f3e5c');

		} catch (e) {console.log(e)}

		try {
			var func3_var0 = 'http://127.0.0.1:8080/input-656_page-1.html#39ee5b94';
		} catch (e) {console.log(e)}
		...
		try {
			var func3_var5 = await func3_var3.arrayBuffer();
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-6310695b] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-61929b92] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '398223b6');
