...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '945bf499');
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '15f67512');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '4da876ff');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '123a9cd1');
		...
		try {
			var func3_var5 = await func3_var0.blob();
		} catch (e) {console.log(e)}

		try {
			var func3_var6 = await func3_var0.text();
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-ae8315da] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-8a50376d] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '84c5eb9f');
		...
		try {
			var func4_var6 = 'http://127.0.0.1:8080/input-3860_page-2.html#a7903a58';
		} catch (e) {console.log(e)}
		...
		try {
			var func4_var7 = await func4_var5.focus();
		} catch (e) {console.log(e)}
