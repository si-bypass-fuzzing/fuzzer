...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '4f78118b');
		...
		try {
			var func0_var2 = await self.cookieStore.get();
		} catch (e) {console.log(e)}

		try {
			var func0_var4 = 'starts-with';
		} catch (e) {console.log(e)}

		try {
			var func0_var3 = {name: self.origin, url: self.origin, matchType: func0_var4, };
		} catch (e) {console.log(e)}

		try {
			var func0_var5 = await self.cookieStore.getAll(func0_var3);
		} catch (e) {console.log(e)}
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '47916c04');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'e1e3269e');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '024cb1a4');
		...
		try {
			var func3_var5 = await func3_var3.postMessage(event,func3_var0);
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-2c6385a0] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-0bd3bc4f] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '5f090b34');
