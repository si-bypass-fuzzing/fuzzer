...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'b10f6945');
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'd78be2e7');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '9411b7ac');
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-72a338f7] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-8ba09bd2] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'ffcf3c31');

		} catch (e) {console.log(e)}

		try {
			var func4_var0 = 'http://127.0.0.1:8080/input-38_page-1.html#e3a31cb6';
		} catch (e) {console.log(e)}
