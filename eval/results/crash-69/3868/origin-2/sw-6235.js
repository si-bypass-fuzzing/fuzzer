...
async function func0(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '33d5a033');
...
async function func1(event) {
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '9860723c');
...
async function func2(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '55765b1d');
...
async function func3(event) {
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, 'fbba37f7');

		} catch (e) {console.log(e)}

		try {
			var func3_var0 = 'http://127.0.0.2:8080/input-40_page-2.html#00f5a40d';
		} catch (e) {console.log(e)}
		...
		try {
			var func3_var5 = 'http://127.0.0.1:8080/input-40_page-2.html#ebf324dd';
		} catch (e) {console.log(e)}
...
async function func4(event) {
        ...
        if (splitPair[0] == 'sanitize' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-2495c8a1] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
		if (splitPair[0] == 'sanitize_partitioned' && splitPair[1] != '127.0.0.2') {
            console.log('[UXSS] [sw-intercept-cookie-partitioned-5cb5a7fe] ' + '127.0.0.2' + ' ' + splitPair[1]);
        }
	}
		...
		try {
			check_site_violation('http://127.0.0.2:8080', location.origin, '0c013f24');
